  ...
  if (allItemsAreShowed) {
      ...
      return (
        <div key={item.id} className="cart__item">
          <div className="cart__item-body">
            <img className="cart__item-img cart__item-img--size " src={item.img} alt={formatName(item.name)} />
    ...
    items = (
      <>
        {items}
        {cartItems.length > 1 && (
          <p className="checkout__more-items" onClick={() => setAllItemsAreShowed(true)}>{`and ${cartItems.length - 1} other item(s)`}</p>
        )}
      </>
    );
  }
